...
# Exploring the intensities of the peptides
...
# Explore Histidine containing peptides
...
# Explore the final selection of peptides
...
ggplot(aes(x = best_parent_m_over_z, y = log(median.total.intensity)),data = selected.peptides )+
        geom_point(aes(color = histidine))+ facet_grid(. ~ median.retentionTime.Bins)+ scale_color_manual(values = c("red","navy"))+
        theme_bw()+ 
        ggtitle("Retention time bins (8 x 11 min bins)")
```

These **`r length(unique(selected.peptides$sequence))`** distinct peptides come from **`r length(unique(selected.peptides$accession_number))`** unique proteins (uniprot acession number).
