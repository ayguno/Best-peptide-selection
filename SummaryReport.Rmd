...
Scores have a near Gaussian distribution with a mean of **`r round(mean(relevant.pep$bestScore),2)`**. We choose to include peptides that have the bestScores above the 20th percentile of the best score distribution:
...
```{r}
library(dplyr)
mean.score <- mean(relevant.pep$bestScore)
relevant.pep <- relevant.pep[relevant.pep$bestScore > mean.score,]
...
simp.peptide$log10.intensity.Bins <- cut(log10(simp.peptide$median.total.intensity),3)
...
peptide.selector <- function(simp.peptide){
        ...
        for(i in seq_along(levels(simp.peptide$median.retentionTime.Bins))){
                ...
                for(k in seq_along(levels(simp.peptide$log10.intensity.Bins))){
                # For a given abundance bin
                 temp_abundance <- temp[temp$log10.intensity.Bins == levels(temp$log10.intensity.Bins)[k],]        
                # Prepare 10 equal m/z bins:
                        temp_abundance$m_over_z_bins <- cut(temp_abundance$best_parent_m_over_z,10)
